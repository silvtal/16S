...
## Input data
data <- "tomate_23sep_SOIL"
main = "~/micro/tomate"
setwd(main)

logs_output_f <- paste0(main,"/logs")

fastaF_input_f  <- paste0("/lustre/proyectos/microbioma/resources/Datos/tomate_2022-09/fastQ/R1_SOIL/") # .fastq.gz files
fastaR_input_f  <- paste0("/lustre/proyectos/microbioma/resources/Datos/tomate_2022-09/fastQ/R2_SOIL/") #

fastaF_output_f <- paste0(main,"/",data,"_dada_filter_F_reads_SOIL/")
fastaR_output_f <- paste0(main,"/",data,"_dada_filter_R_reads_SOIL/")

dada2cleaned_output_f  <- paste0(main,"/",data,"_dada2cleaned_SOIL/")
...
OTU_table_loc <- paste0(main,"/", data, "_otu_table_SOIL.csv")
map_table_loc <- paste0(main,"/", data, "_map_table_SOIL.csv")
...
save_ASVs <- TRUE
ASV_table_loc     <- paste0(main,"/", data, "_asv_table_SOIL.csv")
ASV_tax_table_loc <- paste0(main,"/", data, "_asv_tax_table_SOIL.csv")
...
fastaF_trim_f <- paste0(main,"/",data,"_dada_trim_F_reads_SOIL/")
fastaR_trim_f <- paste0(main,"/",data,"_dada_trim_R_reads_SOIL/")
